...
def train_encoder_izif(opt, generator, discriminator, encoder,
    ...
    os.makedirs(f"results/{opt.run_name}/images_e", exist_ok=True)

    padding_epoch = len(str(opt.n_epochs))
    padding_i = len(str(len(dataloader)))

    batches_done = 0
    losses_imgs, losses_features, e_losses = [], [], []
            ...
            if i % opt.n_critic == 0:
                ...
                if batches_done % opt.sample_interval == 0:
                    ...
                    save_image(reconfiguration_imgs.data[:25],
                               f"results/{opt.run_name}/images_e/{batches_done:06}.png",
                               nrow=5, normalize=True)

                batches_done += opt.n_critic
        losses_features.append(loss_features.item())
        losses_imgs.append(loss_imgs.item())
        e_losses.append(e_loss.item())


    torch.save(encoder.state_dict(), f"results/encoder")
    

    with open(f"results/{opt.run_name}/encoder_losses.csv", mode='w') as file:
        writer = csv.writer(file)
        writer.writerow(["losses_features", "losses_imgs", "e_losses"])
        for i in range(len(losses_features)):
            writer.writerow([losses_features[i], losses_imgs[i], e_losses[i]])
