...
def train_wgangp(opt, generator, discriminator,
    ...
    os.makedirs(f"results/{opt.run_name}/images", exist_ok=True)
            ...
            if i % opt.n_critic == 0:
                ...
                if batches_done % opt.sample_interval == 0:
                    save_image(fake_imgs.data[:25],
                               f"results/{opt.run_name}/images/{batches_done:06}.png",
    ...
    torch.save(generator.state_dict(), f"results/generator")
    torch.save(discriminator.state_dict(), f"results/discriminator")
    with open(f"results/{opt.run_name}/wgan_losses.csv", mode='w') as file:
        writer = csv.writer(file)
        writer.writerow(["generator losses", "discriminator losses"])
        for i in range(len(g_losses)):
            writer.writerow([g_losses[i], d_losses[i]])
